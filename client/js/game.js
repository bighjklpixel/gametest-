...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function confirmPayment(costRequired) {
        ...
        // Send to server
        socket.emit('playCardWithPayment', {
            cardId: selectedCardForAction.id,
            paymentCardIds: selectedCardsForPayment.map(c => c.id)
        });
    ...
    window.gameActions = {
        ...
        declareAttack: (cardId) => {
            socket.emit('declareAttack', { cardId });
            hideActionMenu();
        },
        
        useSamdaeng: (cardId) => {
            socket.emit('useSamdaeng', { cardId });
            hideActionMenu();
        },
        
        useFieldMagic: (cardId) => {
            socket.emit('useFieldMagic', { cardId });
            hideActionMenu();
        },
